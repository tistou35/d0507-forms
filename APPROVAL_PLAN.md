...
| ขั้น | งาน | สถานะ |
|---|---|---|
| 1 | ตำแหน่ง + LINE user ID ในหน้าตั้งค่าระบบ | ✅ ทำแล้ว |
| 2 | ผังผู้อนุมัติรายฟอร์ม (`config/approvals`) | ✅ ทำแล้ว |
| 3 | ตัวแก้ข้อความแจ้งเตือน + ปุ่มดูตัวอย่าง | ✅ ทำแล้ว |
| 4 | ให้ระบบส่งอ่านผังแทน `assignTo` ที่ฝังใน build | ✅ ทำแล้ว |
| 5 | ตีกลับแล้วผู้ส่งแก้ใบเดิมได้ (สถานะ `returned`) | ยังไม่ได้ทำ |
| 6 | ส่งต่ออัตโนมัติเมื่อไม่ตอบ + ตีกลับอัตโนมัติ 7 วัน | ยังไม่ได้ทำ · ต้องมี trigger รายวัน |
| 7 | คิวแจ้งเตือน + Apps Script ยิง LINE | รอ channel access token |
...
### คำตอบที่ได้แล้ว (15 AUG 2026)

1. **ตำแหน่งครบ** — เพิ่มข้อสังเกต: FI/TKI มีหลายคนได้ ฟอร์มที่ระบุชื่อไว้ให้ส่งตรงถึงคนนั้น (โหมด `named`)
2. **ASF → FI** ยืนยันแล้ว
3. **ตีกลับ = ผู้ส่งแก้ใบเดิม** ไม่ใช่เปิดใบใหม่ · เก็บ log ทุกครั้ง
4. **SLA สองชั้น** — ไม่ตอบภายใน N วัน (ตั้งได้) ส่งต่อคนถัดไป · ครบ 7 วัน (ค่าตั้งต้น) ตีกลับอัตโนมัติ
