...
### 4 · ทำแม่แบบ PDF จาก .docx ต้นฉบับ  ← วิธีหลัก

แปลง `.docx` เป็น Google Doc ตรง ๆ แล้วหยอด token ลงในช่องเดิมของเอกสาร
เลย์เอาต์ ตาราง สีหัวตาราง โลโก้ เหมือนกระดาษ 100% เพราะเป็นไฟล์เดียวกัน

```bash
# .docx ต้องอยู่ใน Drive ก่อน — วางใน My Drive/D0507_SOURCE_DOCX ก็ได้
python3 tools/make_tokenmap.py <ABBR>      # อ่าน .docx + นิยามฟอร์ม → gas/TokenMap.gs
python3 tools/gas_push.py push TokenMap ImportTemplate
```

จากนั้นใน Apps Script: แก้ `RUN_LIST` ใน `ImportTemplate.gs` ให้เป็นใบที่ต้องการ
แล้วเปิด `ImportTemplate.gs` → **Run** (เมนูเลือกฟังก์ชันในเว็บกดยาก ใช้ `RUN_LIST` แทน)

ผลลัพธ์คือ `<ABBR>_TEMPLATE` ใน subfolder ของฟอร์มนั้น พร้อมกับ:
- ตาราง **APPROVAL** ต่อท้าย — ระบบมีขั้นอนุมัติที่กระดาษเดิมไม่มี จึงสร้างให้
- บล็อกแดง **TOKEN ที่ยังไม่ได้วาง** ท้ายเอกสาร — เอาไปวางมือแล้วลบบล็อกทิ้ง

**ต้องเปิดเอกสารดูจริง** ทุกครั้ง log บอกแค่จำนวน ไม่ได้บอกว่าวางถูกช่อง

เมื่อกระดาษกับนิยามฟอร์มไม่ตรงกัน 1:1 ให้แก้ที่ `formdefs/_TOKENMAP_OVERRIDES.json`
(`boxes` เรียงช่องติ๊กเอง · `labels` ผูกป้ายเอง · `skip` ตัดที่ไม่ต้องขึ้น PDF)
ทุกรายการเขียน `_why` กำกับว่าทำไมกระดาษถึงไม่ตรง

> ⚠️ ช่องติ๊กจะไม่ถูกแทนอัตโนมัติถ้าจำนวน ☐ ในเอกสารไม่เท่ากับในนิยามฟอร์ม —
> ตั้งใจให้เป็นแบบนั้น วางเลื่อนไปช่องเดียวคือผิดทั้งใบและไม่มีอะไรฟ้อง

<details><summary>วิธีเดิม — เขียนเลย์เอาต์เองด้วย DocumentApp</summary>
...
ใช้เมื่อฟอร์มไม่มี `.docx` ต้นฉบับเท่านั้น — ทำมือช้ากว่าและหน้าตาไม่ตรงกระดาษ
...
```bash
python3 tools/gas_push.py pull              # ดูของบนคลาวด์ก่อนเสมอ
python3 tools/gas_push.py push              # ส่ง gas/*.gs ขึ้นทั้งหมด
python3 tools/gas_push.py push Code         # หรือเลือกเฉพาะไฟล์
```

ส่งผ่าน Apps Script API ตรง ๆ จบในคำสั่งเดียว ไม่ต้องเปิดเว็บ ไม่ต้องวางมือ
`push` จะดึงของบนคลาวด์มาก่อนและ **คงไฟล์ที่ไม่มีในเครื่องไว้** ไม่ลบให้เงียบ ๆ

> 🔴 **อย่าวางโค้ดด้วยมือในเว็บ** — เคยล้มเหลวเงียบ ๆ มาแล้วสองครั้งในโปรเจกต์นี้
> deploy ทับไปทั้งที่โค้ดยังเป็นของเก่า ใช้ `gas_push.py` แทนเสมอ

> ⚠️ `push` เขียนทับทั้งโปรเจกต์ ถ้ามีใครแก้บนเว็บไว้ ให้ `pull` มา diff ก่อน

จากนั้น **Deploy → Manage deployments → ✏️ → Version: New version → Deploy**
