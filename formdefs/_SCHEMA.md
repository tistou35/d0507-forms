...
| `select` | เลือกอย่างเดียว · ปกติเป็นแถวปุ่ม ตั้ง `"menu": true` เมื่อตัวเลือกเยอะจนแถวปุ่มยาวเกินไป (เช่น เลขสำเนาควบคุม 01–99) | `opt[].v` |
...
### 4.2 `mask` — ใส่ตัวคั่นให้ระหว่างพิมพ์

```jsonc
"mask": "AAAA-999999-999"     // A = ตัวอักษร (แปลงเป็นตัวใหญ่) · 9 = ตัวเลข · อื่น ๆ = ตัวคั่น
```

พิมพ์ `as2026084` ได้ `AS-202608-4` — ตัวคั่นโผล่พร้อมตัวถัดไป ไม่ค้างเป็น `AS-` ให้ลบเอง
ผู้ใช้พิมพ์ขีดเองด้วยก็ไม่ซ้อน เพราะตัดตัวคั่นเดิมทิ้งก่อนจัดใหม่ทุกครั้ง

กลุ่มที่ยาวไม่คงที่รองรับได้ (ตัวอักษร 2–4 ตัว) — พิมพ์ตัวที่ไม่เข้าคลาสปัจจุบันเมื่อไร
จะข้ามไปตัวคั่นถัดไปให้เอง

ระหว่างพิมพ์จะเขียนค่าลงในช่องตรง ๆ **ไม่ re-render** ไม่งั้น focus หลุดทุกตัวอักษร

---

## 4.3 การเดินระหว่างแท็บ

กด **ถัดไป** หรือคลิกป้ายแท็บที่อยู่ข้างหน้า จะไปได้ต่อเมื่อ**ช่องบังคับในแท็บปัจจุบันครบแล้ว**
ถ้ายังไม่ครบ ระบบจะทำเครื่องหมายแดงที่ช่องที่ขาด เลื่อนไปช่องแรกที่ขาด และบอกจำนวนที่เหลือ

**ถอยกลับได้เสมอ** — บล็อกเฉพาะการเดินหน้า เพื่อให้ย้อนไปทบทวนของเดิมได้โดยไม่ติดขัด
...
`checklist` คือ **ตารางให้คะแนน** — หัวข้อเป็นแถว ตัวเลือกเป็นคอลัมน์
จำนวนคอลัมน์ตั้งได้ด้วย `opts` ไม่ใส่ = S / U / NA แบบเดิม

```jsonc
{ "type": "checklist",
  "opts": [ { "v": "excellent", "n": { "th": "ดีมาก", "en": "Excellent" } },
            { "v": "good",      "n": { "th": "ดี",    "en": "Good" } } ],
  "items": [ { "id": "TheCourseAsA", "th": "…", "en": "…" } ] }
```

ใช้กับใบที่กระดาษวางเป็นตารางติ๊ก — EFC 2 คอลัมน์ · FTR 3 · SEF กับ EFM 6
ค่าที่เก็บเป็น `{ "<itemId>": "<opt.v>" }` เสมอ

**ตารางแถวซ้ำ `table`**

```jsonc
{ "type": "table", "rows": 8, "max": 20,
  "cols": [ { "k": "title", "type": "text", "w": "40%", "label": { … } },
            { "k": "eff",   "type": "date", "w": "22%", "label": { … } } ] }
```

`rows` = จำนวนแถวว่างที่ขึ้นมารอตั้งแต่แรก (ให้เท่ากับที่กระดาษพิมพ์ไว้)
`max` = เพิ่มได้สูงสุดกี่แถว · ค่าที่เก็บเป็น array ของ object

**ฟังก์ชันในเงื่อนไข**

| เขียน | ได้ |
|---|---|
| `anyStarBelow(3)` | มีข้อ safety-critical ที่ได้เกรดต่ำกว่า 3 |
| `anyBelow(3)` | มีข้อใดก็ได้ที่ได้เกรดต่ำกว่า 3 |
| `filled(k)` | ช่อง `k` มีค่าแล้ว |
| `k has 'x'` | `multi` ที่เลือก `x` ไว้ |

เกรดที่ยังไม่ได้ให้ ไม่นับว่าต่ำกว่าเกณฑ์ — ฟอร์มเปล่าจึงไม่ขึ้นว่าไม่ผ่านตั้งแต่ยังไม่กรอก
